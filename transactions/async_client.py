...
# from py_clib_client.rfq.async_rfq_client import AsyncRfqClient
...
class AsyncClobClient:
    def __init__(
        ...
        self.funder = funder  # Store funder (Safe address) for L1 auth
    ...
    async def create_api_key(self, nonce: int = None) -> ApiCreds:
        ...
        self.logger.debug(f"[CREATE_API_KEY] Address: {self.signer.address()}")
        self.logger.debug(f"[CREATE_API_KEY] Nonce: {nonce}")
        self.logger.debug(f"[CREATE_API_KEY] Headers: {headers}")
    ...
    async def create_or_derive_api_creds(self, nonce: int = None) -> ApiCreds:
        ...
        try:
            ...
            # Fall back to create if derive fails
    ...
    async def post_order(self, order, orderType: OrderType = OrderType.GTC):
        ...
        # DEBUG
        self.logger.debug(f"[POST_ORDER] Signature type: {self.builder.sig_type if self.builder else 'N/A'}")
        self.logger.debug(f"[POST_ORDER] Builder auth available: {self.can_builder_auth()}")

        # Builder flow
        if self.can_builder_auth():
            builder_headers = self._generate_builder_headers(request_args, headers)
            if builder_headers is not None:
                self.logger.debug(f"[POST_ORDER] Using builder headers, keys: {list(builder_headers.keys())}")
                return await post(
        ...
        self.logger.debug(f"[POST_ORDER] Using L2 headers only")
        return await post(
            "{}{}".format(self.host, POST_ORDER),
            headers=headers,
            data=request_args.serialized_body,
    ...
    def can_builder_auth(self) -> bool:
        return self.builder_config is not None
