...
async def test_signature_type(sig_type: int, config: Config, private_key: str):
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"Testing signature type: {sig_type}")
    logger.info(f"{'='*60}")

    signer = Signer(private_key=private_key, chain_id=config.clob.chain_id)

    logger.info(f"Address: {signer.address()}")
    ...
    for key, value in headers.items():
        if key == "POLY_SIGNATURE":
            logger.info(f"  {key}: {value[:20]}...{value[-20:]}")
        else:
...
async def main():
    ...
    logger.info(f"Current Config signature_type: {config.clob.signature_type}")
    logger.info(f"Current Config tx_type: {config.relayer.tx_type}")
    ...
    logger.info("SUMMARY")
    logger.info(f"{'='*60}")
    logger.info(f"Signature Type 1 (POLY_PROXY): {'✅ SUCCESS' if success_1 else '❌ FAILED'}")
    logger.info(f"Signature Type 2 (GNOSIS_SAFE): {'✅ SUCCESS' if success_2 else '❌ FAILED'}")
    ...
    if success_1:
        ...
        logger.info("   1. Private key doesn't match the safe_address")
        logger.info("   2. Account not registered on Polymarket")
        logger.info("   3. Network/API issues")
